...
export default function AdminPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setModal(false); load();
    } catch { toast.error("Failed to save. Check image and try again."); }
    setSaving(false);
  };
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "16px", maxWidth: "480px", width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              <div>
                ...
                <button onClick={() => fileRef.current?.click()} style={{ width: "100%", border: "2px dashed #bfdbfe", background: imgFile ? "#f0fdf4" : "#eff6ff", borderRadius: "8px", padding: "12px", fontSize: "12px", color: imgFile ? "#16a34a" : "#2563eb", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
                  <FiUpload size={14} /> {imgFile ? "✅ " + imgFile.name : "📷 Tap to Upload Photo"}
                </button>
                {imgFile && <div style={{ marginTop: "6px", borderRadius: "8px", overflow: "hidden", height: "120px" }}><img src={URL.createObjectURL(imgFile)} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div>}
              </div>
